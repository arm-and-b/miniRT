...
/*   Updated: 2023/12/17 23:29:48 by mbekouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Test(rays, computing_a_point_from_distance)
{
	...
	ray = create_ray(point(2, 3, 4), vector(1, 0, 0));
...
Test(rays, translating_a_ray)
{
	...
	cr_assert(eq(flt, ray2.direction.x, 0));
	cr_assert(eq(flt, ray2.direction.y, 1));
	cr_assert(eq(flt, ray2.direction.z, 0));
}
...
Test(rays, scaling_a_ray)
{
	...
	cr_assert(eq(flt, ray2.direction.x, 0));
	cr_assert(eq(flt, ray2.direction.y, 3));
	cr_assert(eq(flt, ray2.direction.z, 0));
}
