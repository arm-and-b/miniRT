/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   test_colors.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: abinet <abinet@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/17 16:41:36 by mdias-ma          #+#    #+#             */
/*   Updated: 2023/12/11 21:40:21 by abinet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../MiniRT.h"
#include "utils.h"

/*
 * This test checks if the function "new_color" correctly creates a new color
 * with the given RGB values.
 */
Test(colors, create_colors)
{
	t_color	c;

	c = new_color(1.5, -1.7, 0.7);
	cr_assert(eq(flt, c.r, 1.5));
	cr_assert(eq(flt, c.g, -1.7));
	cr_assert(eq(flt, c.b, 0.7));
}

/* This test checks if the function "add_color" correctly adds two colors
 * together.
 */
Test(colors, add_colors)
{
	t_color	c1;
	t_color	c2;
	t_color	result;

	c1 = new_color(0.9, 0.6, 0.75);
	c2 = new_color(0.7, 0.1, 0.25);
	result = add_colors(c1, c2);
	cr_assert_float_eq(result.r, 1.6, EPSILON);
	cr_assert_float_eq(result.g, 0.7, EPSILON);
	cr_assert_float_eq(result.b, 1.0, EPSILON);
}

/*
 * This test checks if the function "subtract_color" correctly subtracts one
 * color from another.
 */
Test(colors, subtract_colors)
{
	t_color	c1;
	t_color	c2;
	t_color	result;

	c1 = new_color(0.9, 0.6, 0.75);
	c2 = new_color(0.7, 0.1, 0.25);
	result = subtr_colors(c1, c2);
	cr_assert_float_eq(result.r, 0.2, EPSILON);
	cr_assert_float_eq(result.g, 0.5, EPSILON);
	cr_assert_float_eq(result.b, 0.50, EPSILON);
}

/*
 * This test checks if the function "hadamard_product" correctly multiplies
 * two colors together component-wise.
 */
Test(colors, multiply_colors_by_scalar)
{
	t_color	c1;
	t_color	c2;
	t_color	result;

	c1 = new_color(1, 0.2, 0.4);
	c2 = new_color(0.9, 1, 0.1);
	result = hadamard_product(c1, c2);
	cr_assert_float_eq(result.r, 0.9, EPSILON);
	cr_assert_float_eq(result.g, 0.2, EPSILON);
	cr_assert_float_eq(result.b, 0.04, EPSILON);
}
