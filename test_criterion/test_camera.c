...
/*   Updated: 2023/12/15 22:07:43 by mbekouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Test(camera, new_camera)
{
	...
	cr_assert_float_eq(c.fov, M_PI_2, EPSILON);
...
Test(camera, ray_with_transformed_camera)
{
	...
	c = new_camera(201, 101, M_PI_2);
	c.transform = cross_matrices(rotation_y(M_PI_4), translation(0, -2, 5));
	...
	cr_assert_float_eq(r.direction.x, sqrtf(2)/2, EPSILON);
	cr_assert_float_eq(r.direction.y, 0, EPSILON);
	cr_assert_float_eq(r.direction.z, -sqrtf(2)/2, EPSILON);
}
