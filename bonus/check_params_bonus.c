...
/*   Updated: 2024/01/16 21:34:09 by mbekouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_elements(char **element, t_world *world)
{
	...
	else if (!ft_strncmp(element[0], "co", 3))
...
void	check_args(int fd, t_world *world)
{
	...
	if (!world->ambient.parsed)
		ft_error("Ambient Light Missing", true, world);
	if (!world->light.parsed)
		ft_error("Light Missing", true, world);
	if (!world->camera.parsed)
		ft_error("Camera Missing", true, world);
	close(fd);
}
